      ...
      if (err) {
        ...
        if($scope.status) {
          ...
          if($scope.status.availableBalanceStr) {
            $scope.status.availableBalanceStr = $scope.status.availableBalanceStr.replace('btc','SPK');
          }

          // Total Balance
          if($scope.status.totalBalanceStr) {
            $scope.status.totalBalanceStr = $scope.status.totalBalanceStr.replace('btc','SPK');
          }

          // Locked Balance
          if($scope.status.lockedBalanceStr) {
            $scope.status.lockedBalanceStr = $scope.status.lockedBalanceStr.replace('btc','SPK');
          }

          // Pending Balance
          if($scope.status.pendingBalanceStr) {
            $scope.status.pendingBalanceStr = $scope.status.pendingBalanceStr.replace('btc','SPK');
          }

          // Spendable Balance
          if($scope.status.spendableBalanceStr) {
            $scope.status.spendableBalanceStr = $scope.status.spendableBalanceStr.replace('btc','SPK');
          }
        }
      }
  ...
  $scope.showHistory = function() {
    if ($scope.completeTxHistory) {
      ...
      for(var i = 0 ; i < $scope.txHistory.length; i++) {
        $scope.txHistory[i].amountStr = $scope.txHistory[i].amountStr.replace('btc','SPK');
      }
    }
  };
