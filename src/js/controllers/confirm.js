...
angular.module('copayApp.controllers').controller('confirmController', function($rootScope, $scope, $interval, $filter, $timeout, $ionicScrollDelegate, gettextCatalog, walletService, platformInfo, lodash, configService, $stateParams, $window, $state, $log, profileService, bitcore, bitcoreCash, txFormatService, ongoingProcess, $ionicModal, popupService, $ionicHistory, $ionicConfig, payproService, feeService, bwcError, txConfirmNotification, externalLinkService, $http) {
  ...
  $scope.$on("$ionicView.beforeEnter", function(event, data) {
    ...
    try {
      ...
      var message = gettextCatalog.getString('Copay only supports SparksPay using new version numbers addresses');
    ...
    function verifyExcludedUtxos() {
      ...
      if (sendMaxInfo.utxosBelowFee > 0) {
        warningMsg.push(gettextCatalog.getString("A total of {{amountBelowFeeStr}} were excluded. These funds come from UTXOs smaller than the network fee provided.", {
          amountBelowFeeStr: txFormatService.formatAmountStr(wallet.coin, sendMaxInfo.amountBelowFee).replace('btc','SPK')
        }));
      }

      if (sendMaxInfo.utxosAboveMaxSize > 0) {
        warningMsg.push(gettextCatalog.getString("A total of {{amountAboveMaxSizeStr}} were excluded. The maximum size allowed for a transaction was exceeded.", {
          amountAboveMaxSizeStr: txFormatService.formatAmountStr(wallet.coin, sendMaxInfo.amountAboveMaxSize).replace('btc','SPK')
        }));
      }
      return warningMsg.join('\n');
    };

    var msg = gettextCatalog.getString("{{fee}} will be deducted for sparks networking fees.", {
      fee: txFormatService.formatAmountStr(wallet.coin, sendMaxInfo.fee).replace('btc','SPK')
    });
  ...
  $scope.approve = function(tx, wallet, onSendStatusChange) {
    ...
    if ($scope.paymentExpired) {
      popupService.showAlert(null, gettextCatalog.getString('This sparks payment request has expired.'));
      $scope.sendStatus = '';
      $timeout(function() {
        $scope.$apply();
      });
    ...
    getTxp(lodash.clone(tx), wallet, false, function(err, txp) {
      ...
      function confirmTx(cb) {
        ...
        var message = gettextCatalog.getString('Sending {{amountStr}} from your {{name}} wallet', {
          amountStr: tx.amountStr.replace('btc','SPK'),
          name: wallet.name
        });
